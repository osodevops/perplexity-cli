...
use super::types::{AgentRequest, AgentResponse, ApiErrorResponse};

impl ApiClient {
    /// Send an agent completion request via POST /responses.
    pub async fn agent_completion(
        &self,
        request: &AgentRequest,
    ) -> Result<AgentResponse, PplxError> {
        ...
        let resp = response.json::<AgentResponse>().await?;
        Ok(resp)
    }
}
